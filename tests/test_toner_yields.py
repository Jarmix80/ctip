...
class TonerYieldStoreTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        def register_timezone(connection, _record):
            """Udostępnia SQLite funkcję używaną przez znaczniki czasu konta PostgreSQL."""
    ...
    async def test_toner_permission_coexists_with_crm_and_survives_older_client(self):
        """Scalenie zachowuje niezależne prawa tonerów i ustawienia powiadomień CRM."""
        async with self.sessions() as session:
            user, _ = await admin_users.create_user(
                session,
                email="tonery@example.com",
                first_name=None,
                last_name=None,
                internal_ext=None,
                role="operator",
            ...
            self.assertTrue(user.can_edit_toner_yields)
            self.assertTrue(user.crm_sales_email_enabled)
            self.assertFalse(user.crm_operations_sms_enabled)
            await admin_users.update_user(
                ...
                crm_sales_email_enabled=False,
                crm_operations_sms_enabled=True,
                crm_operations_email_enabled=False,
                mobile_phone=None,
                firebird_app_user_id=None,
                firebird_app_user_login=None,
            )
            await session.commit()
            await session.refresh(user)
            self.assertTrue(user.can_edit_toner_yields)
            self.assertFalse(user.crm_sales_email_enabled)
            self.assertTrue(user.crm_operations_sms_enabled)
