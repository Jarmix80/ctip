...
class BnpCatalogResult:
    """Wynik utworzenia kartoteki magazynowej wykupu."""
...
class BnpBuyoutResult:
    """Wynik atomowej finalizacji wykupu urządzenia."""
    ...
    *,
    ...
    try:
        ...
        cursor.execute(
            """
            INSERT INTO MAGAZYN (
                ID_ODDZIAL, ID_FIRMA, ID_MAGAZYN, RODZAJ, INDEKS, NAZWA, JM,
                ILOSC, DATA_Z, MARKA, MODEL, ID_MODEL, SERIAL, IDVAT, VAT_STAWKA,
                SYNWP, V_2010A, UWAGI
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    *,
    ...
    try:
        ...
        cursor.execute(
            ...
            """,
            (BNP_BUYOUT_WAREHOUSE_ID, normalized_index.upper(), warehouse_item_id),
        )
        if cursor.fetchone() is not None:
            raise ValueError("Na magazynie 27 istnieje już inna kartoteka o podanym indeksie.")

        marker = _text(f"CTIP WYKUP BNP {normalized_document} {kto}", 1000)
        cursor.execute(
            """
            UPDATE MAGAZYN
            SET INDEKS = ?, NAZWA = ?, VAT_STAWKA = ?, IDVAT = COALESCE(IDVAT, 1), UWAGI = ?
            WHERE ID_MAGAZYN_TABLE = ? AND ID_MAGAZYN = ?
            """,
            (
                normalized_index,
                normalized_name,
        ...
        document_number = _next_pz_document_number(cursor, year=document_date.year)
        pz_number = _text(f"PZ / {document_number} / {document_date.year}", 30)
        cursor.execute(
            """
            INSERT INTO ZAKUPY (
                ID_ODDZIAL, ID_FIRMA, ID_KLIENT, NAZWA, ADRES, KOD, POCZTA, NIP,
                RODZAJ_DOK, DOKUMENT, NUMER, ID_MP, ID_MW, DOK_ZEW, DATA_WYST,
                DATA_PRZY_WYDA, DATA_PLAT, PLATNOSC, MIEJSCE_WYST, WYSTAWIL,
                ODBRUTTO, NETTO1, VAT1, BRUTTO1, SUMA_NETTO, SUMA_VAT,
                SUMA_BRUTTO, UWAGI
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        cursor.execute(
            ...
            INSERT INTO ZAKPOZYCJA (
                ID_ZAKUPY, ID_FIRMA, ID_KLIENT, ID_MAGAZYN, RODZAJ_DOK, NUMER,
                DATA_PRZY_WYDA, ID_MP, ID_MW, RODZAJ, INDEKS, NAZWA, CENA_NETTO,
                ILOSC, JM, WARTOSC_NETTO, CENA_Z, WARTOSC_Z, STAWKA_VAT, VAT,
                IDVAT, WARTOSC_BRUTTO, POBRANO, SERIAL, EWIDENCJA, UWAGI
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        cursor.execute(
            "UPDATE MASZYNA SET EWIDENCJA = ? WHERE ID_MASZYNA_TABLE = ?",
            (normalized_target, machine_table_id),
        )
        cursor.execute(
            "SELECT COALESCE(ILOSC, 0) FROM MAGAZYN WHERE ID_MAGAZYN_TABLE = ?",
            (warehouse_item_id,),
        )
