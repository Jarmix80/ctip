...
def _require_catalog_mutations() -> None:
    """Blokuje zmiany katalogu podczas odczytowej fazy wdrożenia."""
    if not settings.shipping_catalog_mutations_enabled:
        raise HTTPException(
            status_code=status.HTTP_423_LOCKED,
            detail="Zmiany katalogu Shipping są tymczasowo zablokowane.",
        )


def _require_fulfillment() -> None:
    """Blokuje operacje wysyłkowe i zapisy MS do czasu jawnego pilota."""
    if not settings.shipping_fulfillment_enabled:
        raise HTTPException(
            status_code=status.HTTP_423_LOCKED,
            detail="Realizacja wysyłek jest tymczasowo zablokowana.",
        )
...
async def shipping_config(
    ...
    firebird_write_ready, firebird_write_reason = firebird_writes_enabled()
    ...
    return {
        "shipping": {
            ...
            "catalog_mutations_enabled": settings.shipping_catalog_mutations_enabled,
