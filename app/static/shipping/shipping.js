...
function renderShippingBulkActions() {
  ...
  document.getElementById("shipping-generate-selected").disabled = shippingState.bulkBusy || readyCount === 0;
  document.getElementById("shipping-generate-consolidated").disabled = shippingState.bulkBusy || !canConsolidate;
  document.getElementById("shipping-generate-ready").disabled = shippingState.bulkBusy;
...
function applyShippingCase(caseData, showTrackingFeedback = true) {
  ...
  document.getElementById("shipping-review").disabled = Boolean(shipment)
    || Boolean(orderState && !orderState.can_review);
  document.getElementById("shipping-create").disabled = !ready || locationBlocked
    || Boolean(orderState && !orderState.can_prepare_shipment);
  document.getElementById("shipping-manual").disabled = !ready || locationBlocked
    || Boolean(orderState && !orderState.can_prepare_shipment);
  document.getElementById("shipping-close-order").disabled = status !== "shipment_created"
  ...
  document.getElementById("shipping-catalog-stock").innerHTML = rows.length
    ? rows.map((item) => {
      ...
      return `<tr>
        ...
        <td><button type="button" class="shipping-button secondary compact" data-manual-item="${itemId}">Mapuj</button></td>
      </tr>`;
    }).join("")
...
async function enrichCatalogWithWeb() {
  ...
  try {
    ...
    button.disabled = !shippingState.config?.compatibility_web?.enabled;
  }
}
...
function renderManualSelection() {
  ...
  document.getElementById("shipping-manual-save").disabled = count === 0;
}
...
async function initializeShipping() {
  ...
  try {
    ...
    dpdDemoButton.hidden = !(config.dpd.enabled && config.dpd.mode === "demo" && config.dpd.api_ready && config.dpd.sender_ready);
    ...
    document.getElementById("shipping-catalog-web").disabled = !config.compatibility_web?.enabled;
    ...
    if (config.after_cutoff) shippingAlert("Jest po godzinie granicznej. Upewnij się, że kurier nie zakończył dzisiejszego odbioru.");
    await loadShippingQueue(true);
