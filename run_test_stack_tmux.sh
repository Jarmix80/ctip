...
FB_HOST_VALUE="$(read_env_value "FB_HOST")"
if [[ -z "${FB_HOST_VALUE}" ]]; then
    FB_HOST_VALUE="127.0.0.1"
fi
FB_PORT_VALUE="$(read_env_value "FB_PORT")"
if [[ -z "${FB_PORT_VALUE}" ]]; then
    FB_PORT_VALUE="3050"
fi

firebird_reachable=false
if nc -z "${FB_HOST_VALUE}" "${FB_PORT_VALUE}" >/dev/null 2>&1; then
...
case "${START_FIREBIRD}" in
    ...
    never)
        ...
        ;;
    auto)
        if [[ "${FB_HOST_VALUE}" == "127.0.0.1" || "${FB_HOST_VALUE}" == "localhost" || "${FB_HOST_VALUE}" == "::1" ]]; then
            should_start_firebird=true
        fi
        ;;
    *)
        echo "Nieznana wartość START_FIREBIRD=${START_FIREBIRD}. Użyj auto|always|never." >&2
        exit 1
        ;;
esac

if [[ "${firebird_reachable}" != "true" && "${should_start_firebird}" == "true" ]]; then
    if ! command -v docker >/dev/null 2>&1; then
        echo "Firebird ${FB_HOST_VALUE}:${FB_PORT_VALUE} nie odpowiada, a docker nie jest dostępny do uruchomienia kontenera ${FIREBIRD_CONTAINER}." >&2
        exit 1
    fi

    if ! docker ps -a --format '{{.Names}}' | grep -Fxq "${FIREBIRD_CONTAINER}"; then
        echo "Nie znaleziono kontenera Firebird '${FIREBIRD_CONTAINER}'." >&2
        exit 1
    fi
    ...
    if [[ "${running_state}" != "true" ]]; then
        ...
        echo "Kontener Firebird już działa: ${FIREBIRD_CONTAINER}"
    fi

    echo "Czekam na Firebird 127.0.0.1:${FB_PORT_VALUE} (max ${FIREBIRD_WAIT_SECONDS}s)..."
    for _ in $(seq 1 "${FIREBIRD_WAIT_SECONDS}"); do
        if nc -z 127.0.0.1 "${FB_PORT_VALUE}" >/dev/null 2>&1; then
            firebird_reachable=true
            break
        fi
        sleep 1
...
if [[ "${firebird_reachable}" != "true" ]]; then
    echo "Firebird nie odpowiada na ${FB_HOST_VALUE}:${FB_PORT_VALUE}. Uruchom serwer Firebird lub ustaw START_FIREBIRD=always." >&2
    exit 1
fi
...
Uruchomiono sesję tmux '${SESSION_NAME}' (lokalne środowisko testowe ${ENV_FILE}):
  - firebird  (${FB_HOST_VALUE}:${FB_PORT_VALUE})
