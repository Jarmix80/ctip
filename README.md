...
- `POST /admin/contracts/forms/{id}/workflow/status` – zapisuje ręczny status GRENKE po stronie CTIP (`WAITING_SIGNATURE`, `APPROVED_ORDER`, `REJECTED_GRENKE`, `RENTAL_WITHOUT_GRENKE`); odmowa nie usuwa historii, tylko ustawia 7-dniowy termin zwolnienia zasobów, a decyzje końcowe (`APPROVED_ORDER`, `RENTAL_WITHOUT_GRENKE`) ustawiają 14-dniowy termin archiwizacji.
