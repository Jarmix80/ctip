...
Kanoniczny testowy na `192.168.0.9:8000` wyrównano 12 września 2026 r. do produkcyjnego dashboardu. Działa obraz `ctip/test-runtime:6be022468c021db2118b60db496f75c406ce5adb` z worktree `.codex/shipping-retry-local-validation-test-worktree`; kod Shipping, telemetrii i wykupu BNP jest zgodny z produkcją, a CRM/LAB/Delivery/Bot Identity pozostają dodatkami testowymi. Obie bazy mają rewizję `b7e2d4f6a810`; danych biznesowych nie resetowano. Polecenia `ctiptest server-*` należy wykonywać wyłącznie z tego worktree, nie z archiwalnego katalogu głównego. Bieżący odbiór, backupy i cztery wcześniej znane błędy testów FLOW/raportów opisuje `docs/instal/test_server_runtime.md` w worktree testowym. Dla `server-status` należy jawnie wskazać podany `CTIP_TEST_IMAGE`.
