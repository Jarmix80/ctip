...
- Wyrównanie z produkcyjnym dashboardem tonerów `ef79672` odebrano 12 września 2026 r.; obraz testowy `6be0224`, obie bazy na `b7e2d4f6a810`. Wszystkie 15 usług działa; 63 wspólne pliki bez różnic, 48 testów zmiany poprawnych. Cztery wcześniej znane błędy testów FLOW/raportów pozostają odrębne. Dodatkowe moduły testowe i izolacja zachowane; odbiór i rollback: `docs/instal/test_server_runtime.md`.
...
- Odbiór telemetrii: 298 CSV zachowanych w archiwach, 15325 wiadomości rozliczonych, 25461 okresów MS dla 1001 aktywnych urządzeń, 25954 migawki V i 630 PrintRadar. Ponowienie bez nowych punktów dziennych; 977 testów poprawnych, cztery znane wcześniej testy wyłączone. Importer wydań tonerów z dokumentów i wykresy pozostają kolejnym etapem.
- Miesięczna historia MS CPC korzysta z tych samych tabel telemetrycznych, bez kopiowania faktur: trzy lata okresów urządzeń obecnie na aktywnych umowach, osobne liczniki początku i końca, historyczny klient oraz indeksowana kontrola jakości historii. Konfiguracja i potwierdzone klucze MS: `docs/instal/telemetria_urzadzen.md`.
- Pozyskiwanie danych urządzeń z Remote CSV/DPLAC, poczty, V-Maintenance i PrintRadar: `docs/instal/telemetria_urzadzen.md`. Oddzielny worker `app.telemetry_worker`, podgląd `/admin/telemetry`, migracje addytywne do `e8c7d6a5b410`; DPLAC są archiwizowane po zapisie. Moduł wymaga jawnej konfiguracji, bez zmian w Shipping. V-Maintenance używa osobnego połączenia UTF8; pola z NUL są zachowywane bezstratnie, bez zmiany MS.
...
- Poprzednie wyrównanie testowego CTIP wdrożono 10 września 2026 r. w obrazie `902d0556f5580ad58833b6ef0088977e797faa69`, zachowując CRM, LAB i Bot Identity; automat uzgadniania MS pozostaje wyłączony w testowym. Historyczny odbiór i rollback: `docs/instal/test_server_runtime.md`.
...
- Hotfix pełnego zamknięcia zleceń Shipping, naprawa `DATA_Z` i operatora, rozróżnienie statusu „zamknięte” oraz procedury korekty historycznej i przerwanego zamknięcia dnia: `docs/instal/hotfix_shipping_pelne_zamkniecie_2026-09-08.md`, `scripts/repair_shipping_day_close.py`.
- Automatyczne uzgadnianie zgodnych ręcznych zamknięć Shipping z MS, blokowanie konfliktów i procedura produkcyjnego dry-runu: `docs/instal/wdrozenie_shipping_uzgadnianie_ms_2026-09-08.md`, `scripts/reconcile_shipping_ms.py`.
...
- `app/services/workflow_machine_binding.py` – automat dla statusu `APPROVED_ORDER`: przed zapisem całego pakietu wymaga, aby istniejąca kartoteka `MASZYNA` należała do klienta magazynowego `FB_WAREHOUSE_CLIENT_ID` (domyślnie `656`), a następnie wiąże urządzenia workflow z klientem w `MASZYNA.ID_KLIENT`, wymusza `AKTYWNA=TAK` i `SYNWP=1`, próbuje znormalizować `MASZYNA.EWIDENCJA` do `KP/<numer>/GRENKE/<reszta>` oraz tworzy brakujące rekordy `MASZYNA` z mapowaniem danych z tabeli `MODEL`; właściciel inny niż magazyn, brak `ID_KLIENT` albo wieloznaczne dopasowanie blokuje całą operację bez częściowego zapisu. Dla źródła `firebird_magazyn_28` automat dociąga bieżący rekord `MAGAZYN`, parsuje techniczne `NAZWA` (`S/N`, `nr.wew`) i normalizuje warianty modeli Ricoh (`IMC` -> `IM C`, `MPC` -> `MP C`) zanim dopasuje `MODEL`; jeśli `MAGAZYN.ID_MODEL` jest pusty lub błędny, ale istniejąca jednoznacznie dopasowana kartoteka ma poprawne `MASZYNA.ID_MODEL`, używa tego identyfikatora jako bezpiecznego źródła zapasowego. Status zwracany do `/genform` pokazuje też licznik `powiązane/wszystkie` i skrót pierwszych błędów z identyfikatorem urządzenia (`producent`, `model`, `serial` albo `ewidencja`). Lista formularzy izoluje błąd odszyfrowania pojedynczego historycznego rekordu: zachowuje podstawowe dane kontaktowe, oznacza niedostępny payload i wyłącza akcje wymagające jego treści zamiast zwracać błąd całego dashboardu.
...
Jednorazową korektę formularza `70` wykonuje się wyłącznie na serwerze produkcyjnym po wdrożeniu przypiętego commita i utworzeniu pełnego backupu PostgreSQL oraz Firebird. Skrypt zapisuje raporty i dziennik wznawialnej operacji w `runtime/form70_device_replacement`, wymaga tokenu z bezpośredniego dry-runu i nie usuwa proformy `52/proforma/2026`. Lokalny adres PostgreSQL `127.0.0.1` lub `localhost` jest akceptowany wyłącznie na hoście posiadającym produkcyjny adres `192.168.0.8`. Na docelowym urządzeniu skrypt toleruje wyłącznie pustą proformę albo nieaktywne pozostałości starej proformy, bez identyfikatorów formularza i sprawy; inne powiązanie nadal blokuje zapis. Dokładna procedura, walidacja oraz rollback są opisane w `docs/instal/korekta_formularza_70_2026-09-08.md`.
...
| `SHIPPING_DPD_FIREBIRD_MILESTONES_ENABLED` | `false` | Po potwierdzonych zdarzeniach InfoServices zapisuje datę odbioru i doręczenia oraz istotny opis przesyłki do MS; wymaga aktywnego zapisu Firebird. |
...
| `SHIPPING_MS_RECONCILE_ENABLED` | `false` | Włącza niezależny kontroler aktywnych przesyłek. Pełna zgodność ręcznego zamknięcia powoduje przejęcie dokumentów do CTIP bez SMS-a i e-maila, a rozbieżność ustawia `reconcile_required`; kontroler nigdy nie zapisuje Firebirda. |
| `SHIPPING_MS_RECONCILE_INTERVAL_SECONDS` | `300` | Odstęp kontroli ręcznych zmian MS; dopuszczalny zakres wynosi 60–3600 sekund. |
| `SHIPPING_MS_RECONCILE_BATCH_LIMIT` | `250` | Maksymalna liczba aktywnych przesyłek kontrolowanych w jednym cyklu. |
| `SHIPPING_GEOCODER_ENABLED` | `false` | Udostępnia ręczny przycisk sprawdzenia pocztowej części adresu w Adresy.app. |
| `ADDRESY_APP_API_URL` | `https://api.adresy.app/api/v1` | Oficjalny endpoint HTTPS; inne hosty są odrzucane. |
| `ADDRESY_APP_API_KEY` | *(puste)* | Klucz używany wyłącznie po stronie serwera w nagłówku Bearer; nie trafia do przeglądarki ani logów. |
...
- Widok `/device` ma osobne trasy `/device/intake`, `/device/warehouse`, `/device/history` i `/device/issues`. Dostęp wymaga jawnie nadanej sekcji `device`; utworzenie modelu, dostawcy lub PZ wymaga dodatkowo mapowania konta CTIP do użytkownika Menadżera Serwisu. Formularz przyjęcia korzysta z jednego uporządkowanego układu z numerowanymi sekcjami, przyciskiem `+ Dodaj urządzenie`, wyszukiwaniem na bieżącej liście oraz przyklejonym podsumowaniem liczby urządzeń, kompletności i wartości dokumentu. Zachowuje model i dostawcę wybranych z list `ID | opis`, pokazuje wynik przy akcji dodawania, a następnie czyści pola modelu i ceny, przywraca liczbę `1` oraz ustawia fokus na wyborze kolejnego modelu. Usunięcie pozycji z listy automatycznie przelicza numery KP pozostałych urządzeń od aktualnej sugestii Firebirda, bez pozostawiania luk. Tworzenie nowej kartoteki modelu jest wydzielone z formularza PZ i znajduje się w obszarze audytu pod tabelą magazynu. Próba utworzenia PZ prezentuje przy przycisku pełną listę braków, oznacza błędne pola na czerwono i przenosi fokus do pierwszego problemu. Profil testowy udostępnia dodatkowo trasę `/device/intake/prototypes` z trzema statycznymi makietami; widok nie ładuje skryptu modułu urządzeń, nie wywołuje API i jest niedostępny w profilu produkcyjnym.
- Osobna trasa `/device/bnp-buyout`, widoczna w menu bezpośrednio pod `Przyjęcie PZ`, obsługuje wykup pojedynczego urządzenia po zakończeniu wynajmu BNP. Wyszukiwanie po serialu pokazuje klienta, ewidencję i kartoteki magazynowe; pole nazwy pozycji otrzymuje edytowalną podpowiedź `Producent Model S/N:serial`. Układ docelowy wykorzystuje wariant panelu operacyjnego: formularz dokumentu i akcje znajdują się po lewej, a przyklejony kafelek znalezionego urządzenia po prawej. Pola formularza mają zwiększoną wysokość i czcionkę, a akcje utworzenia kartoteki ze stanem `0` oraz finalizacji PZ są wyróżnione osobnymi kartami opisującymi dokładny skutek zapisu. Poprawne `KP/<numer>/...` zachowuje dotychczasową procedurę i domyślny indeks `WKP/<numer>/BNP`. Dla każdego urządzenia z pustą lub niestandardową ewidencją formularz wyświetla ostrzeżenie i podpowiada `WKP/<serial>` w obu polach, np. `WKP/B630205697`; operator może niezależnie edytować dopiski po `/`, zachowując bazowy serial. Brakującą kartotekę tworzy się na magazynie `27` ze stanem `0`. Finalizacja pobiera numer i datę dokumentu, nazwę oraz cenę netto, tworzy jednoelementowy PZ, ustawia docelowe `MASZYNA.EWIDENCJA` i wymaga końcowego stanu `1`, bez tworzenia rekordu `SERIAL`. Ochrona wcześniejszego wykupu obejmuje oba warianty WKP. Pełne reguły opisuje `docs/projekt/wykup_bnp.md`. Profil testowy udostępnia dodatkowo `/device/bnp-buyout/prototypes` z trzema nieaktywnymi wariantami wyglądu: karty etapowe, panel operacyjny i kreator 3 kroków.
- Przyjęcie batch jest idempotentne: jeden UUID odpowiada dokładnie jednemu żądaniu i jednemu dokumentowi PZ. Ponowienie niezmienionego formularza zachowuje UUID, a zmiana serialu, KP, ceny, modelu, dostawcy lub danych dokumentu automatycznie generuje nowy UUID. Obowiązkowa data przyjęcia jest wspólna dla wszystkich pozycji i steruje datą przyjęcia PZ, datą `MAGAZYN.DATA_Z` oraz rokiem numeracji dokumentu. Nagłówek zawiera także osobną datę wystawienia, formę płatności (`Przelew`, `Gotówka` albo `Pobranie`) i edytowalny termin płatności, domyślnie 14 dni od daty wystawienia. Każda pozycja tworzy osobny `MAGAZYN` oraz `MASZYNA` przypisaną początkowo do klienta magazynowego `656`; nie są tworzone nowe rekordy `SERIAL`. Opcjonalne liczniki B/W, kolor i skan aktualizują bieżące pola `MASZYNA`, trafiają do historii `ctip.device_counter_reading` oraz do kolumn `LICZNIK B/W`, `LICZNIK KOLOR` i `LICZNIK SKAN` arkusza.
- Wycofanie PZ zachowuje historyczne wpisy `ctip.device_inventory_unit`, ale unikalność serialu i numeru KP obejmuje tylko urządzenia aktywne. Ponowne przyjęcie wcześniej wycofanego egzemplarza tworzy nowy aktywny wpis i nie usuwa historii; wyszukiwanie po tożsamości preferuje aktualny aktywny rekord. Arkusz może zastąpić starszą tożsamość w wierszu KP wyłącznie wtedy, gdy jej serial i opcjonalny identyfikator magazynowy odpowiadają najnowszemu wycofanemu wpisowi CTIP dla tego KP.
...
- `GET /genform` – osobny ekran handlowca do generowania i podglądu formularzy; domyślny widok `Formularze w toku` pokazuje aktywne sprawy, opcja `Wszystkie` łączy formularze aktywne i archiwalne, a jedno pole wyszukuje w wybranej kategorii po ID formularza, firmie, NIP, e-mailu, telefonie, kliencie Menadżera Serwisu i numerze proformy. Wiadomości historyczne pozostają osobnym widokiem z własnymi filtrami.
- `GET /flow` – główny widok roboczy FLOW dla obsługi umów i urządzeń po zalogowaniu.
- `GET /device`, `/device/intake`, `/device/bnp-buyout`, `/device/warehouse`, `/device/audit`, `/device/history`, `/device/issues` – ekrany modułu obsługi urządzeń; testowy `GET /device/intake/prototypes` pokazuje trzy nieaktywne makiety formularza PZ, `GET /device/bnp-buyout/prototypes` trzy warianty formularza wykupu, `GET /device/style-prototypes` cztery warianty tła obszaru roboczego, `GET /device/sidebar-prototypes` trzy warianty lewego menu, a `GET /device/header-prototypes` trzy warianty górnego panelu.
...
- `GET /admin/device/bnp-buyout/lookup`, `POST /admin/device/bnp-buyout/catalog`, `POST /admin/device/bnp-buyout/complete` – podgląd wykupu BNP z trybem identyfikacji `kp`/`serial` i ostrzeżeniami, przygotowanie kartoteki magazynu `27` ze stanem `0` oraz atomowa finalizacja PZ i docelowej ewidencji `WKP`. Pole `expected_ewidencja` w żądaniach zapisu jest wymagane, ale dopuszcza pusty ciąg.
...
- `GET /contracts`, `GET /admin/contracts/dashboard` – dashboard „Obsługa umów” i dane integracyjne (formularze SUBMITTED, Firebird, pozycje `MAGAZYN` dla magazynu `28`), wymagane uprawnienie sekcji `generator`; endpoint wspiera `include_devices=true|false` (domyslnie `true`) do kontrolowania kosztownego odczytu pozycji magazynowych oraz `archive_scope=all|active|accepted|rejected|unfilled|ksero_partner|closed_other` dla menu archiwum GenForm. Wartość `all` zwraca aktywne formularze i wszystkie kategorie archiwalne, nie obejmuje natomiast osobnego archiwum wiadomości GRENKE.
...
- `POST /admin/contracts/forms/{id}/workflow/devices` – zapisuje wybór urządzeń bez kasowania i ponownego tworzenia zachowanych rekordów, dzięki czemu nie traci identyfikatora `MASZYNA`; blokuje cały pakiet przy kolizji z aktywnym FLOW, rezerwacją ręczną albo kartoteką `MASZYNA` należącą do klienta innego niż magazynowy `656`, zachowuje wcześniej wybrany egzemplarz chwilowo niedostępny w magazynie oraz aktualizuje wyłącznie osobne pola rezerwacji arkusza.
...
- `docs/instal/hotfix_shipping_pelne_zamkniecie_2026-09-08.md` – pełne zamknięcie zleceń w MS, kontrola `DATA_Z` i operatora oraz bezpieczna naprawa rekordów historycznych.
- `docs/instal/wdrozenie_shipping_uzgadnianie_ms_2026-09-08.md` – cykliczna kontrola aktywnych przesyłek, automatyczne przejęcie zgodnych ręcznych zamknięć i blokowanie rozbieżności bez zapisu Firebirda.
