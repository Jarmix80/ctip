...
- `GET /orbit/prototypes` – dostępna wyłącznie w profilu `.env.test` galeria pięciu niefunkcjonalnych i responsywnych kierunków pulpitu KP Orbit: Klasyczny Orbit, Centrum Operacyjne, Proces 360, Kafelki Modułowe oraz Minimal Focus. Każda makieta prezentuje komplet modułów CTIP i dane demonstracyjne, nie ładuje JavaScriptu, nie wywołuje API i nie udostępnia aktywnych przejść do paneli; w profilu produkcyjnym trasa zwraca `404`.
- `GET /orbit/simple-prototypes` – testowa galeria wybranego kierunku KP Orbit: pięć rozwinięć konsoli bocznej `/choice` (Pulpit dnia, Kolejki operacyjne, Oś dnia, Radar kierownika i Monitor kompaktowy) oraz wybrany, dzielony ekran logowania. Każdy wariant wyboru pokazuje datę i czas, dziewięć modułów w rozwijalnych grupach oraz demonstracyjne informacje o formularzach, wysyłkach i urządzeniach. Lewa część logowania jest wyśrodkowana, nie zawiera statycznego logotypu i wykorzystuje powiększoną animację CSS z obsługą `prefers-reduced-motion`. Makiety nie wywołują API, nie zapisują danych i są niedostępne poza profilem `.env.test`.
