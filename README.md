...
- `app/web/shipping_ui.py` + `app/templates/shipping/` + `app/static/shipping/` + `app/api/routes/admin_shipping.py` + `app/services/shipping_*` – moduł `/shipping` w docelowym układzie V2 do katalogu zgodności części, realizacji wysyłek DPD, dokumentów RW/WZ/FV i przeszukiwalnego Archiwum; poprzedni interfejs pozostaje pod `/shipping/legacy`, a operacje są chronione niezależnymi flagami etapowego wdrożenia.
