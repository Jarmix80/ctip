...
- Osobna trasa `/device/bnp-buyout`, widoczna w menu bezpośrednio pod `Przyjęcie PZ`, obsługuje wykup pojedynczego urządzenia po zakończeniu wynajmu BNP. Wyszukiwanie po serialu pokazuje klienta, KP i kartoteki magazynowe; pole nazwy pozycji otrzymuje edytowalną podpowiedź `Producent Model S/N:serial`. Układ docelowy wykorzystuje wariant panelu operacyjnego: formularz dokumentu i akcje znajdują się po lewej, a przyklejony kafelek znalezionego urządzenia po prawej. Pola formularza mają zwiększoną wysokość i czcionkę, a akcje utworzenia kartoteki ze stanem `0` oraz finalizacji PZ są wyróżnione osobnymi kartami opisującymi dokładny skutek zapisu. Brakującą kartotekę `WKP/<numer>/BNP` można utworzyć na magazynie `27` ze stanem `0`. Finalizacja pobiera numer i datę dokumentu, nazwę oraz cenę netto, tworzy jednoelementowy PZ, zmienia `MASZYNA.EWIDENCJA` z `KP` na `WKP` i wymaga końcowego stanu `1`, bez tworzenia rekordu `SERIAL`. Profil testowy udostępnia dodatkowo `/device/bnp-buyout/prototypes` z trzema nieaktywnymi wariantami wyglądu: karty etapowe, panel operacyjny i kreator 3 kroków.
...
- `GET /admin/device/bnp-buyout/lookup`, `POST /admin/device/bnp-buyout/catalog`, `POST /admin/device/bnp-buyout/complete` – podgląd wykupu BNP, przygotowanie kartoteki magazynu `27` ze stanem `0` oraz atomowa finalizacja PZ i zmiany `KP` na `WKP`.
