...
- Rozszerzenie telemetrii: wszystkie CSV wraz z archiwami, kolejka IMAP do folderów `przetworzone`/`odrzucone`, dopasowanie aktywnych umów oraz dwuletnie migawki dzienne V-Maintenance i PrintRadar z osobnymi zdarzeniami. Harmonogram docelowy: codziennie 23:55 Europe/Warsaw; szczegóły i odbiór w `docs/instal/telemetria_urzadzen.md`.
- Odbiór telemetrii 11 września 2026 r.: zapisano 25461 okresów MS dla 1001 urządzeń aktualnie na aktywnych umowach oraz 1886 rekordów pilota pozostałych źródeł. Zadania cykliczne i pełna historia pozostałych źródeł nie są jeszcze uruchomione; zakres filtrów wymaga ustalenia. Historia wydań tonerów z MS została potwierdzona odczytem, ale jej importer pozostaje kolejnym etapem. Szczegóły: `docs/instal/telemetria_urzadzen.md`.
...
- Pozyskiwanie danych urządzeń z Remote CSV/DPLAC, poczty, V-Maintenance i PrintRadar: `docs/instal/telemetria_urzadzen.md`. Oddzielny worker `app.telemetry_worker`, podgląd `/admin/telemetry`, migracja addytywna `a6d9e1f3b520`; DPLAC pozostają na miejscu. Moduł domyślnie wyłączony, bez zmian w Shipping. V-Maintenance używa osobnego połączenia UTF8; pola z NUL są zachowywane bezstratnie, bez zmiany MS.
