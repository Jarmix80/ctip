...
- Ekran `/genform` udostępnia akcje `Wyświetl`/`Dodaj urządzenie`/`Stwórz proformę`/`Dezaktywuj` dla każdego wniosku oraz okno szczegółów: dla statusu `SUBMITTED` prezentowane są odszyfrowane dane klienta w układzie zgodnym z końcowym podsumowaniem formularza, z przyciskiem `Kopiuj` przy każdym polu oraz akcjami `Drukuj` i `PDF`; modal `Dodaj urządzenie` pokazuje tabelę pozycji Firebird z filtrami, rezerwacją arkusza Google i ręcznym wpisem cen `netto/brutto`, a modal `Proforma` pokazuje osobno dane firmy z formularza oraz aktualnego nabywcy dokumentu (`bank` albo `klient z formularza`), pozwala zapisać PDF bez otwierania podglądu A4 i usuwać samą proformę bez zwalniania rezerwacji urządzeń. Dla pozostałych statusów widoczna jest czytelna informacja operacyjna (np. „formularz został wysłany, ale nie został jeszcze wypełniony”).
...
- `GET /contracts`, `GET /admin/contracts/dashboard` – dashboard „Obsługa umów” i dane integracyjne (formularze SUBMITTED, Firebird, pozycje `MAGAZYN` dla magazynu `28`), wymagane uprawnienie sekcji `generator`; endpoint wspiera `include_devices=true|false` (domyslnie `true`) do kontrolowania kosztownego odczytu pozycji magazynowych.
...
- `POST /admin/contracts/forms/{id}/workflow/status` – zapisuje reczny status biznesowy sprawy po stronie CTIP (`Robocza`, `Oczekuje na akceptacje`, `Zaakceptowano`, `Zerowka`, `Odrzucono`); przy `REJECTED` wykonywana jest automatyczna proba zwolnienia rezerwacji arkusza GRENKE.
