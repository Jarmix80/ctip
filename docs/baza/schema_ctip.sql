...
CREATE INDEX idx_shipping_shipment_archive_search_trgm
    ...
    WHERE status = 'closed'::text;
CREATE INDEX idx_shipping_shipment_provider_tracking
    ON ctip.shipping_shipment USING btree (provider, tracking_number);
...
CREATE TABLE ctip.shipping_tracking_parcel (
    id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    provider text DEFAULT 'dpd'::text NOT NULL,
    waybill text NOT NULL,
    source_channel text NOT NULL,
    latest_business_code text,
    latest_description text,
    latest_event_time timestamp with time zone,
    latest_depot text,
    latest_depot_name text,
    latest_country text,
    replacement_waybill text,
    status_category text DEFAULT 'other'::text NOT NULL,
    status_label text DEFAULT 'Inny status DPD'::text NOT NULL,
    ...
    created_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL,
    updated_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL,
    CONSTRAINT uq_shipping_tracking_parcel_waybill UNIQUE (provider, waybill),
    CONSTRAINT shipping_tracking_parcel_category_check
        CHECK (status_category = ANY (ARRAY[
            'registered'::text, 'in_transit'::text, 'out_for_delivery'::text,
            'pickup_ready'::text, 'delivered'::text, 'undelivered'::text,
            'redirected'::text, 'returning'::text, 'critical'::text, 'other'::text
        ]))
);

CREATE INDEX idx_shipping_tracking_parcel_status_event
    ON ctip.shipping_tracking_parcel USING btree (status_category, latest_event_time DESC);
CREATE INDEX idx_shipping_tracking_parcel_attention_event
    ON ctip.shipping_tracking_parcel USING btree (requires_attention, latest_event_time DESC);

CREATE TABLE ctip.shipping_tracking_event (
    id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    parcel_id integer REFERENCES ctip.shipping_tracking_parcel(id) ON DELETE CASCADE,
    source_event_key text NOT NULL,
    semantic_event_key text,
    canonical_event_id integer REFERENCES ctip.shipping_tracking_event(id) ON DELETE SET NULL,
    waybill text,
    dpd_event_id text,
    object_id text,
    business_code text,
    ...
    event_data json DEFAULT '[]'::json NOT NULL,
    raw_payload json DEFAULT '{}'::json NOT NULL,
    is_cancelled boolean DEFAULT false NOT NULL,
    cancelled_at timestamp with time zone,
    received_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL,
    CONSTRAINT uq_shipping_tracking_event_source_key UNIQUE (source_event_key),
    CONSTRAINT shipping_tracking_event_operation_check
        CHECK (operation_type = ANY (ARRAY['INSERT'::text, 'CANCEL'::text]))
);

CREATE INDEX idx_shipping_tracking_event_parcel_time
    ON ctip.shipping_tracking_event USING btree (parcel_id, event_time DESC, id DESC);
CREATE INDEX idx_shipping_tracking_event_object
    ON ctip.shipping_tracking_event USING btree (object_id);
CREATE INDEX idx_shipping_tracking_event_semantic
    ON ctip.shipping_tracking_event USING btree (semantic_event_key);
CREATE INDEX idx_shipping_tracking_event_canonical
    ON ctip.shipping_tracking_event USING btree (canonical_event_id);

CREATE TABLE ctip.shipping_tracking_sync_run (
    id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    source_channel text NOT NULL,
    ...
    error_text text,
    triggered_by integer REFERENCES ctip.admin_user(id) ON DELETE SET NULL,
    started_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL,
    completed_at timestamp with time zone,
    CONSTRAINT shipping_tracking_sync_run_trigger_check
        CHECK (trigger_type = ANY (ARRAY['scheduler'::text, 'manual'::text, 'backfill'::text])),
    CONSTRAINT shipping_tracking_sync_run_status_check
        CHECK (status = ANY (ARRAY[
            'processing'::text, 'success'::text, 'partial'::text, 'failed'::text
        ]))
);

CREATE INDEX idx_shipping_tracking_sync_started
    ON ctip.shipping_tracking_sync_run USING btree (started_at DESC);
...
GRANT SELECT,INSERT,DELETE,UPDATE ON TABLE ctip.shipping_tracking_parcel TO appuser;
GRANT SELECT,INSERT,DELETE,UPDATE ON TABLE ctip.shipping_tracking_event TO appuser;
GRANT SELECT,INSERT,DELETE,UPDATE ON TABLE ctip.shipping_tracking_sync_run TO appuser;
...
GRANT ALL ON SEQUENCE ctip.shipping_tracking_parcel_id_seq TO appuser;
GRANT ALL ON SEQUENCE ctip.shipping_tracking_event_id_seq TO appuser;
GRANT ALL ON SEQUENCE ctip.shipping_tracking_sync_run_id_seq TO appuser;
