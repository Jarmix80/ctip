...
- Wycofane urządzenia pozostają w historycznym rejestrze CTIP, ale nie blokują ponownego przyjęcia tego samego serialu albo numeru KP. Unikalność jest wymuszana wyłącznie pomiędzy aktywnymi wpisami, a wyszukiwanie operacyjne zawsze preferuje wpis aktywny. Przy ponownym użyciu KP worker arkusza może nadpisać starszy serial lub `MS_ID_MAGAZYN_TABLE` tylko po zgodności z najnowszym wycofanym wpisem CTIP dla tego KP; każda inna rozbieżność nadal zatrzymuje synchronizację.
