...
- Synchronizacja rezerwacji FLOW do Google Sheets jest sterowana konfiguracja runtime z `/admin -> Konfiguracja bazy -> Google Sheets (FLOW)`; panel zapisuje namespace `google_sheets` (`enabled`, `credentials_path`, `spreadsheet_id`, `workflow_devices_worksheet`), akceptuje w polu `spreadsheet_id` surowe ID albo pelny URL Google Sheets, udostepnia test polaczenia przez `POST /admin/google-sheets/test` oraz przygotowanie brakujacych naglowkow FLOW przez `POST /admin/google-sheets/bootstrap-headers`; dla arkusza legacy bez prawdziwego naglowka bootstrap umie wstawic nowy pierwszy wiersz naglowkow, przesunac dane w dol i sformatowac naglowek (zamrozenie, filtr, kolory, szerokosci). Dodatkowa zmienna `GOOGLE_SHEETS_CONFIG_LOCK=true` blokuje modyfikacje runtime przez `PUT /admin/config/google-sheets` (HTTP `423`) i loguje probe zmiany w audycie (`config_google_sheets_update_blocked_lock`). Statusy urzadzen do modalu `/flow` nie sa juz pobierane na zywo przy kazdym otwarciu: backend utrzymuje lokalny cache w tabeli `ctip.workflow_sheet_status_cache`, odswiezany recznie przez `POST /admin/contracts/workflow/sheet-status-refresh` i automatycznie schedulerem w tle.
- Tabela `/genform` pokazuje w kolumnie `Status GRENKE` etykietę bieżącego statusu oraz pod spodem datę ostatniej zmiany statusu (`status_changed_at`).
- Modal workflow pokazuje osobno `Poczatek GRENKE` oraz `Poczatek KP`: pierwsza data pochodzi z pierwszej akceptacji `APPROVED_ORDER`/historycznego `APPROVED` (dla mailboxa z daty wiadomości), druga z terminu dowozu `delivery_date`.
...
- `Wyświetl` otwiera modal szczegółów z czytelnym układem danych firmy i reprezentantów (zgodnym z końcowym podsumowaniem formularza klienta), bez pustej przestrzeni w widoku wydruku/PDF, z przyciskiem kopiowania przy każdym polu oraz akcjami `Dane zostały wpisane`, `Wniosek GRENKE`, `Drukuj` i `PDF`; przycisk `Wniosek GRENKE` jest aktywny wyłącznie po etapie `PROFORMA_CREATED` i otwiera nowe okno z prefillem danych przez backend (`full` albo `partial`), a fallback `partial` koduje query zgodnie z parserem `decodeURI` po stronie GRENKE; tryb `full` uzupełnia również dane dostawcy (`providerName`, `providerPostal`, `providerNip`) z danych sprzedawcy zapisanej proformy.
...
| Generator formularzy | `/genform`, `/admin/forms`, `/admin/forms/{id}`, `/admin/contracts/forms/{id}/workflow/grenke-launch`, `/formularz/{token}` |
| Obsługa dostaw | `/delivery`, `/admin/delivery/cases`, `/admin/delivery/grenke-contracts`, `/admin/delivery/grenke-contracts/reminders/run` |
