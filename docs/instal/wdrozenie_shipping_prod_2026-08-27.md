...
4. kompilację kodu oraz lekkie testy ustawień i grafu migracji; testy wartości domyślnych czyszczą odziedziczone zmienne procesu, skrypt dodatkowo izoluje flagi Shipping i ocenia polecenia Python według kodu wyjścia, ponieważ `unittest` zapisuje prawidłowy postęp również na stderr;
